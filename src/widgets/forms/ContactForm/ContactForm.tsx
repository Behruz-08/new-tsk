...
import { contactFormSchema, type ContactFormData } from '@/features/forms/model/validations';
import { formsService } from '@/features/forms/services/forms.service';
import { useCacheInvalidation, useSubmitFormMutation } from '@/shared/hooks/useApi';
import { useValidatedForm } from '@/shared/hooks/useForm';
  ...
  const handleFormSubmit = async (data: ContactFormData) => {
    try {
      ...
      if (localFile && !data.file) {
        setFileUploadError('Дождитесь завершения загрузки файла');
        return;
      }
  ...
  return (
    <form onSubmit={handleSubmit(handleFormSubmit)} className={className}>
      <div className={styles.form}>
        ...
        {(submitFormMutation.isError || fileUploadError) && (
          <div className={styles.submitError} role="alert">
            <MessageSquare size={16} />
            <span>{fileUploadError || 'Ошибка отправки формы'}</span>
          </div>
        )}
        ...
        <Button
          ...
          size="lg"
          fullWidth
          loading={submitFormMutation.isPending || isFileUploading}
          disabled={
            !isValid ||
          ...
          leftIcon={
            !submitFormMutation.isPending && !isFileUploading && <MessageSquare size={18} />
          }
        >
          {isFileUploading
            ? 'Загрузка файла...'
